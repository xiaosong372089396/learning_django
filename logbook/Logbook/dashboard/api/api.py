...
class tools(object):
    ...
    def get_host(self):
        ...
        data = {
            ...
            "method": "host.get",
            "params": {
                "output": ["hostid", "name", "ip"],     # ["hostid", "name"]
...
#        print "删除主机: \033[40m%s\031[0m \tid :\033[31m%s\033[0m" % (Hostid, res['result']['hostids'])
    ...
    #   返回所有hostid==10251的主机，并查询name包含CPU, Usage, 字段的item， 并按照name排序
    def get_hostid_item(self, hostid):
        print "*******************************"
        data = json.dumps(
            {
                "jsonrpc": "2.0",
                ...
                "params": {
                    ...
                    "hostids": hostid,
                    "search": {
                        "key_": "tcp_connection[ESTABLISHED]",
                    },
                    "sortfield": "name",
                },
                "auth": self.authid,
    ...
    def get_history_data(self, itemid):
        print "*********************"
        import time, datetime
        threeDayAgo = (datetime.datetime.now() - datetime.timedelta(days=29))
        Yest = int(time.mktime(threeDayAgo.timetuple()))
        Today = int(time.time())
        data = json.dumps(
              {
                  "jsonrpc": "2.0",
        ...
        res = html_json['result']
        for host in res:
            print "\t", "Itemid:", host['itemid'], "\t", "Item_value:", host['value'], "\t", "clock:", time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(int(host['clock'].encode('UTF8'))))
            #print host
...
while 1:
    ...
    if input == 3:
        ...
        # test get items
        hostidss = int(raw_input("\nYour selection >"))
        zabbix.get_hostid_item(hostidss)
        # test history item
        historyid = str(raw_input("\nYour selection >"))
        zabbix.get_history_data(historyid)
        #####
        print "#######################"
        break
    if input == 4:
